...
## V7 — 조정(이월 수익) 타입 추가 (2026-06)

입출금 구분에 **조정(이월)** 타입 추가. 기록 시작 전(예: 작년)에 발생한 수익처럼 "실제 현금에는 있지만 원금은 아닌" 금액을 입력하는 용도.

- 자동 현금 = 순입금액 + **조정** − 보유원가 + 실현손익
- 순입금액(원금)에는 조정이 포함되지 않으므로 수익률 분모가 왜곡되지 않음
- 계좌 수익 = 평가금액 − 순입금액 → 이월 수익까지 누적 수익으로 정확히 표시

사용법: 입출금 추가 → 구분을 "조정(이월)"로 선택 → 작년 수익액 입력 (예: 6,000,000 / 메모: 2025년 수익 이월)

---

## V6 — 차트 높이 동기화 (2026-06)

종목별 수익률 카드와 손익 추이 카드의 높이를 동일하게 묶음. 수익률 바 목록은 카드 높이에 맞춰 간격이 균등 분배되어 하단 여백 없이 채워지고, 손익 추이 차트도 카드 높이를 가득 채움.

---

## V5 — 수익률 차트 개선 (2026-06)

| # | 변경 내용 |
|---|---|
| 1 | 종목별 수익률 차트 — 0% 축이 데이터에 맞춰 동적으로 이동 (전부 마이너스면 축이 오른쪽 끝 → 빈 절반 없이 전체 폭 사용). 종목 수에 따라 바 두께·간격·글자 크기가 자동으로 촘촘해짐 (8개 이하 / 14 / 22 / 그 이상 4단계) |
| 2 | 계좌 섹션 2컬럼 → 원래 1줄(전폭 세로) 배치로 복원 |
...
| # | 변경 내용 |
|---|---|
| 1 | 평가금액 수식 점검 — 화면에 수식 명시(`현재 평가금액 = 주식 평가 + 현금`, `수익 = 평가금액 − 순입금액`). 수동 현금 모드일 때 왜곡 경고 표시. 비중% 계산이 종목 수정에 반응하지 않던 useMemo 의존성 버그 제거 |
| 2 | 원/달러 환율 입력을 '주식 포트폴리오' 제목 옆 인라인으로 이동 (기존 환율 카드 삭제, 환율 0이면 ⚠ 표시) |
| 3 | '티커별 합산 (전체)' 섹션 삭제 |
| 4 | 계좌별 입금/출금 현황 접기/펴기 — 기본 접힘(요약만), '내역 N건' 버튼으로 펼침, 입출금 추가 시 자동 펼침 |
| 5 | 계좌 섹션 가로 2컬럼 배치 (종목 테이블은 카드 내 가로 스크롤) |
